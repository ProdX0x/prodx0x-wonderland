...
export function createWorld(container,apps){
 ...
 const stages=apps.map((app,i)=>{
   ...
   return{stage,device,createDevice,z,x,baseY,isWide};
 });
 // Near foreground islands give parallax and frame the first view.
 const firstIsland=createTerrain(19,6.2,128,'#98ad89');firstIsland.position.set(-23,-1,0);scene.add(firstIsland);scene.add(makeTree(-15,1.1,-3,2.3,42));scene.add(makeTree(-21,1.8,4,1.6,16));
 ...
 renderer.domElement.addEventListener('webglcontextlost',event=>{event.preventDefault();paused=true;document.getElementById('fallback').hidden=false;});renderer.domElement.addEventListener('webglcontextrestored',()=>{paused=false;last=performance.now();document.getElementById('fallback').hidden=true;});
 return{setProgress(p){target=THREE.MathUtils.clamp(p,0,Math.max(0,apps.length-1));},getProgress(){return current;},retryVideo(){stages[active]?.device?.activate(true);},dispose(){cancelAnimationFrame(frame);window.removeEventListener('resize',resize);document.removeEventListener('visibilitychange',visibility);stages.forEach(s=>s.device?.dispose());scene.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();}});scene.environment.dispose();renderer.dispose();renderer.domElement.remove();}};
}
